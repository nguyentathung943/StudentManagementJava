...
public class FillPassForm extends JFrame {
	...
	public static void main(String[] args) throws SQLException {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FillPassForm frame = new FillPassForm();
	 ...
	 * @throws SQLException 
	 */
	public FillPassForm() throws SQLException {
		...
		JLabel lblNoti1 = new JLabel("First time logging in, type in your credentials so we can settle things up for you");
		lblNoti1.setForeground(Color.WHITE);
		lblNoti1.setFont(new Font("Arial", Font.BOLD, 20));
		lblNoti1.setBounds(20, 70, 770, 74);
		contentPane.add(lblNoti1);
		
		JLabel lblPasswordNoti = new JLabel("Password must be longer than 5 characters");
		...
		btnCheck.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(id.equals("")) {
					...
					try {
						ResultSet a = ServerConnection.ExecuteQuery("select * from credential where username='"+id+"'");
						if(a.next()) {
							lblUsername.setText("");
							if(!(a.getString("password")==null)) {
								lblUsername.setText("This account is already have a password");
							}
							else {
		...
		lblNewLabel_1.setIcon(new ImageIcon(FillPassForm.class.getResource("/icon/back.png")));
		...
		lblID.setBounds(166, 217, 524, 24);
		...
		btnLogin.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(newPass.equals("")) {
					...
					lblMessage.setText("");
					try {
						ServerConnection.ExcecuteQueryUpdate("update credential set password='"+newPass+"' where username='"+id+"'");
						dispose();
						LoginForm form  = new LoginForm();
						form.setVisible(true);
